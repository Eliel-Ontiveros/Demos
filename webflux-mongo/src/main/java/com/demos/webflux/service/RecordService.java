package com.demos.webflux.service;

import com.demos.webflux.entity.RecordDocument;
import com.demos.webflux.repository.PrimaryRecordRepository;
import com.demos.webflux.repository.SecondaryRecordRepository;
    ...
    private Mono<Long> seedRepository(boolean primary) {
        ...
        return Flux.range(0, batches)
                .concatMap(batchIndex -> {
                    ...
                    return primary
                            ? primaryRecordRepository.insertAll(batch).thenReturn((long) batch.size())
                            : secondaryRecordRepository.insertAll(batch).thenReturn((long) batch.size());
                })
                .reduce(0L, Long::sum);
    }
