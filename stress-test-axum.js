/**
 * ╔══════════════════════════════════════════════════════════╗
 * ║          BREAKING POINT STRESS TEST — AXUM (Rust)        ║
 * ║  Objetivo: Encontrar el límite máximo del framework       ║
 * ║  Puerto  : 8084                                           ║
 * ║  Ejecución: k6 run stress-test-axum.js                    ║
 * ╚══════════════════════════════════════════════════════════╝
 *
...
export const options = {
    ...
    thresholds: {
        // Informativos: muestran dónde rompe, no cancelan la prueba
        "error_rate": ["rate<0.50"],     // Alerta si >50% errores
        "http_req_duration": ["p(95)<10000"],   // Alerta si p95 > 10 s
        "post_duration": ["p(99)<15000"],   // Alerta si p99 POST > 15 s
        "get_duration": ["p(99)<15000"],   // Alerta si p99 GET  > 15 s
    },
};
...
export function stressGet() {
    // Variar offset para evitar lecturas triviales del mismo bloque
...
export function handleSummary(data) {
    return {
        ...
        "stress-axum-results.json": JSON.stringify(data, null, 2),
    };
}
